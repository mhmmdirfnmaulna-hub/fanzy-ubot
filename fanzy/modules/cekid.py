...
__HELP__ = """
<b>🆔 CEK ID TOOLS</b>
├ <code>{0}cekid</code> - Mengetahui ID User
╰<b>Reply pesan/ketik username</b>
"""
...
async def generate_profile_card(client: Client, user: User):
    ...
    if has_profile_photo and os.path.exists(profile_photo_path):
        ...
        profile_photo_path = None # Set None jika tidak ada file yang didownload
    ...
    # Return dua path agar bisa dihapus di fungsi utama
...
async def cekidte(client, message):
    ...
    # Menerima tuple (kartu, foto profil)
    profile_card_path, photo_path = await generate_profile_card(client, target_user)
    ...
    msg = f"""
<blockquote><b>╭「 𝗜𝗡𝗙𝗢𝗥𝗠𝗔𝗦𝗜 𝗨𝗦𝗘𝗥 」</b>
│ 👤 <b>Username:</b> {username_text}
│ 🆔 <b>ID User:</b> {target_user.id} 
│ 🏷 <b>Jumlah ID:</b> {digit_info}
│ 〽️ <b>DC ID</b> <code>{target_user.dc_id or 'Tidak diketahui'}</code>
│ ✨️ <b>Premium:</b> { "True ✅" if getattr(target_user, 'is_premium', False) else "False ❌"}
│ 🔗 <b>Link Profil:</b> <a href="tg://user?id={target_user.id}">Click Here</a>
╰───<b>「 Fanzy Userbot 」</b> </blockquote>
"""
    ...
    if profile_card_path:
        ...
        # Hapus file kartu profil
        if os.path.exists(profile_card_path):
            os.remove(profile_card_path)

        # Hapus file foto profil yang didownload tadi
