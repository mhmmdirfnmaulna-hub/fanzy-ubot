...
async def get_vars(user_id: int, vars_name: str, query: str = "vars") -> Optional[Union[int, str]]:
    result = await varsdb.find_one({"_id": user_id})  # ✅ TANPA read_preference
...
async def add_to_vars(user_id: int, vars_name: str, value: int, query: str = "vars"):
    """Menambahkan value ke list dalam vars (VERSI FORCE)"""
    # Ambil data yang sudah ada
    vars_data = await get_vars(user_id, vars_name, query) or ""
    
    # Konversi ke list
    if vars_data:
        if isinstance(vars_data, str):
            vars_list = [int(x) for x in vars_data.split() if x]
    ...
    if value not in vars_list:
        ...
        # Simpan ulang dengan force
        new_value = " ".join(map(str, vars_list))
        await set_vars(user_id, vars_name, new_value, query)
        
        # VERIFIKASI LANGSUNG
        cek = await get_vars(user_id, vars_name, query) or ""
        if str(value) in str(cek):
            print(f"[SUCCESS] add_to_vars: {value} berhasil ditambahkan ke {vars_name}")
            return True
        else:
            print(f"[ERROR] add_to_vars: {value} GAGAL ditambahkan ke {vars_name}, mencoba lagi...")
            # Percobaan kedua
            await set_vars(user_id, vars_name, new_value, query)
            return False
